...
//========================================================================
// 函数: void IapRead(u16 EE_address,u8 *DataAddress,u16 number)
...
//========================================================================
// 函数: void IapErase(u16 EE_address)
// 描述: 把指定地址的EEPROM扇区擦除.
...
void IapErase(u16 EE_address)
{
	...
	IapTrig();					  //触发EEPROM操作
	DisableIap();				  //禁止EEPROM操作
}

//========================================================================
// 函数: void IapWrite(u16 EE_address,u8 *DataAddress,u16 number)
...
void IapWrite(u16 EE_address, u8 *DataAddress, u16 number)
{
	...
	for (bytesCnt = 0; bytesCnt < number; bytesCnt++)
	{
		IAP_ADDRH = EE_address / 256;	  //送地址高字节（地址需要改变时才需重新送地址）
		IAP_ADDRL = EE_address % 256;	  //送地址低字节
		IAP_DATA = DataAddress[bytesCnt]; //送数据到IAP_DATA，只有数据改变时才需重新送
		IapTrig();						  //触发EEPROM操作
		EE_address++;					  //下一个地址
	}									  //直到结束
	DisableIap();
